...
│   │       └── payroll_events.py
│   ├── features/
│   │   ├── hr/                  # ★ THIS MODULE — part 1 (HR owns its models)
│   │   │   ├── models/          #     hr-scoped tables (moved here)
│   │   │   │   ├── erp_department.py
│   │   │   │   ├── erp_employee.py
│   │   │   │   ├── erp_leave_type.py
│   │   │   │   ├── erp_leave_balance.py
│   │   │   │   ├── erp_leave_request.py
│   │   │   │   └── erp_leave_movement.py
│   │   │   ├── router.py
│   │   │   ├── schemas.py
│   │   │   ├── service.py
│   │   │   ├── ports.py
│   │   │   └── repository.py
│   │   └── payroll/             # ★ THIS MODULE — part 2 (payroll owns its models)
│   │       ├── models/          #     payroll-scoped tables (moved here)
│   │       │   ├── erp_compensation.py
│   │       │   ├── erp_payroll_run.py
│   │       │   ├── erp_payroll_entry.py
│   │       │   └── erp_payroll_settings.py
│   │       ├── router.py
│   │       ├── schemas.py
│   │       ├── service.py
│   │       ├── ports.py
│   │       └── repository.py
│   ├── models/
│   │   └── base.py              # shared DeclarativeBase + UUID/Timestamp mixins
│   ├── cli.py
│   ├── main.py
│   └── seed.py                   # reference data + per-tenant leave-type defaults
├── alembic/versions/             # 0001_initial (base/RPC/RLS), 0002_hr_payroll
...
```
api (router.py)  →  features/<module>/service.py  →  features/<module>/repository.py  →  features/<module>/models
                          │
...
- Cross-feature calls **between `features/hr` and `features/payroll`** go through `ports.py` interfaces (e.g. payroll reads approved unpaid leave via `LeaveLedgerPort`). Neither feature imports the other's repository or service. **The one documented exception:** `features/payroll/models` reference `erp_employee` in `features/hr/models` (one-way, for the composite FK `erp_payroll_entries → erp_employees`); all runtime business access still crosses via `ports.py`.
...
- **Where these keys are registered:** `services/identity/src/identity/core/permissions.py` — constants + `CATALOG` + `PERMISSION_MODULES` (the module docstring: *"A permission must be added here AND via migration before it can be assigned to roles"*). Plus a new identity Alembic migration inserting the keys (`ON CONFLICT (key) DO NOTHING`), and `identity/core/constants.py` `SYSTEM_ROLE_DEFINITIONS`:
...
- `ref_id UUID NULL` — the reference id: the `leave_year` for `annual_accrual` (the Rule 4 idempotency probe), the leave request id for `leave_request`, or the adjustment id for `manual_adjustment`
- `occurred_at TIMESTAMPTZ NOT NULL` — when the movement happened (the ledger's timestamp)
...
```
  ...
  The unique index blocks only **identical** periods. Overlapping-but-different periods (e.g. `Jan 1–31` vs `Jan 15–Feb 15`) are blocked by the **service-level overlap check** (§4.10); a Postgres `btree_gist` exclusion constraint is deferred.
...
- Employee: `active ⇄ on_leave`; `active → terminated` is terminal (a terminated employee cannot be re-activated or re-hired via this endpoint — a new employee record is required). Leave requests and payroll runs cannot be created for terminated employees after `termination_date`.
...
Alembic under `services/core/alembic/`. Migration **`0002_hr_payroll`** (after `0001_initial` base/RPC/RLS scaffolding):
...
### 4.10 Rule 10 — No overlapping active payroll runs

1. `create_run` (and re-`compute`) validates in the service that no existing **non-void** run's `[period_start, period_end]` overlaps the new period. Overlap → `PayrollPeriodConflictError` (409 `payroll-period-conflict`, §7).
2. The partial unique index (§3.2) only blocks **identical** periods; the application-layer check is the hard guarantee for overlaps (a Postgres exclusion constraint is deferred).
3. Voided runs are excluded from the check — a void frees the period for a new `draft` run (§3.3).
...
### 6.1 Shared prerequisites (already in `services/core`)

The `services/core` skeleton provides: `core/config.py` (`CORE_` settings: `DATABASE_URL`, `JWT_PUBLIC_KEY`, `JWT_ISSUER`, `CORE_DEFAULT_CURRENCY=USD`, `CORE_PAYROLL_ROUNDING=nearest`), `core/security.py` (identity JWT verification), `core/tenant_resolver.py` + `core/tenant_context.py`, `api/deps.py` (`get_tenant_context`, `get_current_user`, `require_permission`), `db/session.py` + `db/base.py` (engine + `after_begin` RLS hook + mixins), `domain/value_objects.py` (**`Money`**), `core/permissions.py` and `core/audit_events.py`. Verify with: `uv run ruff check services/core`, a `require_permission("erp.hr.read")`-protected endpoint returning 401 without token, 403 without the permission, 200 with it.

### Step 1 — Models + migration

Write the 10 models per §3.2 — the 6 HR models in `features/hr/models/`, the 4 payroll models in `features/payroll/models/` (§2.1). Then `alembic revision` → `0002_hr_payroll` implementing §3.4. **Verify the hard part:** run the two-tenant integration assertions (§9) against the migration — RLS must block cross-tenant writes and silently filter cross-tenant reads.

### Step 2 — `features/hr`

- `features/hr/models/` — `erp_department.py`, `erp_employee.py`, `erp_leave_type.py`, `erp_leave_balance.py`, `erp_leave_request.py`, `erp_leave_movement.py` (§2.1, §3.2).
...
- `features/payroll/models/` — `erp_compensation.py`, `erp_payroll_run.py`, `erp_payroll_entry.py`, `erp_payroll_settings.py` (§2.1, §3.2). These reference `erp_employee` in `features/hr/models/` for the composite FK (§2.2).
...
- **Audit actions ⊇ emitted events:** every audit action is recorded; only the §2.5 topics are published as events after commit.
...
| POST | `/api/v1/payroll/runs` ✓ | `erp.payroll.write` | non-overlapping period (service check, §4.10) |
...
The generic proxy `apps/web/src/app/api/v1/[...path]/route.ts` forwards every `/api/v1/*` call to a backend via `callBackend`. The `hr` and `payroll` path segments must be routed to `services/core` (added to the shared `CORE_SEGMENTS` set in `apps/web/src/lib/server/auth.ts`):

```ts
// apps/web/src/lib/server/auth.ts — CORE_SEGMENTS
...
Component conventions (follow the existing code): server components render the shell + `PageHeader`; client components ("use client") do data fetching with `useSession()` + the feature API clients; mutations use optimistic UI + `ApiError` surfaced as inline/toast errors; status badges/empty states/skeletons reuse `@/components/ui/*`; **permission gating is UI-only here** — the real gate is backend `require_permission`.

### 8.4 Sidebar

`apps/web/src/components/dashboard/app-sidebar.tsx`: add an ERP *People* group — *Employees*, *Departments*, *Leave* → shown when `permissions` contain `erp.hr.read`; *Payroll*, *Compensation*, *Settings* → shown when `erp.payroll.read`.
...
- **Overlap:** create a run for an overlapping period → 409 `payroll-period-conflict`; after `void`, the same period is allowed again

### 9.3 Acceptance criteria (Definition of Done)

- [ ] Migration `0002_hr_payroll` applies up and down; RLS verified by isolation tests
...
- Employment-status transitions to/from `on_leave` (nothing sets it in Phase 1 — employees move only `active → terminated`)

---

## 11. Related

- `docs/architecture/erp-phase1.md` — parent architecture (shared RLS/events/permissions policy); **pending — to be created**
