...
import { ACCOUNT_TYPE_LABELS, formatMoney, toMoney } from "@/lib/finance/format";
  ...
  return (
    <div className="flex flex-wrap items-end gap-3">
      ...
      <Button type="button" onClick={onRun} disabled={running}>
        {running ? <LoaderCircle aria-hidden="true" className="size-4 animate-spin" /> : null}
        Run
      </Button>
    </div>
  );
...
type StatementRow =
  ...
  | { kind: "net"; label: string; amount: number | string; negative?: boolean };
    ...
    <div className="overflow-hidden rounded-xl border border-border bg-card">
      <table className="w-full text-sm">
        <tbody>
          {!hasLines ? (
            <tr>
              <td className="px-4 py-8 text-center text-sm text-muted-foreground">{emptyLabel}</td>
            </tr>
          ) : (
            rows.map((row, index) => {
              if (row.kind === "section") {
                return (
                  <tr key={index} className="border-b border-border/60 bg-muted/40">
              ...
              return (
                <tr key={index} className="border-b border-border/40 last:border-b-0">
                  <td className={cn("px-4 py-2", labelClass)}>{row.label}</td>
                  <td className={cn("px-4 py-2 text-right", amountClass)}>
...
const tbColumns: FinanceColumn<TrialBalance["rows"][number]>[] = [
  { label: "Code", render: (row) => <code className="font-mono text-xs">{row.code}</code> },
  ...
  { label: "Credit", align: "right", render: (row) => formatMoney(row.credit) },
  {
    label: "Balance",
    align: "right",
    render: (row) => <span className="tabular-nums">{formatMoney(toMoney(row.debit) - toMoney(row.credit))}</span>,
  },
];
...
function TrialBalanceView({ periods }: { periods: FiscalPeriod[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      />
      <ReportToolbar
        fields={[{ id: "tb-asof", label: "As of", value: asOf, onChange: setAsOf }]}
        onRun={() => void run(asOf)}
        running={loading}
      />
      {loading ? <TableSkeleton rows={6} /> : null}
      {error ? <FinanceErrorState message={error} onRetry={() => void run(asOf)} /> : null}
      {data && !loading ? (
        <FinanceTable
          columns={tbColumns}
          rows={data.rows}
          ...
          footer={
            <span className="flex justify-between gap-4">
              ...
              <span className="tabular-nums">
                {formatMoney(data.total_debit)} / {formatMoney(data.total_credit)}
              </span>
            </span>
          }
        />
...
function ProfitAndLossView({ periods }: { periods: FiscalPeriod[] }) {
  ...
  const run = useCallback(async (from: string, to: string) => {
    ...
    try {
      ...
      setError(errorMessage(err, "Could not load the profit and loss statement."));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (error) return <FinanceErrorState message={error} onRetry={() => void run(fromDate, toDate)} />;
  if (!data) return null;
  ...
  return (
    <div className="space-y-5">
      ...
      <ReportToolbar
        fields={[
          { id: "pnl-from", label: "From", value: fromDate, onChange: setFromDate },
          { id: "pnl-to", label: "To", value: toDate, onChange: setToDate },
        ]}
      ...
      <StatementTable
        rows={[
          ...
          { kind: "total", label: "Total revenue", amount: data.total_revenue },
          ...
          { kind: "total", label: "Total expenses", amount: data.total_expenses },
          {
            kind: "net",
            label: "Net income",
...
function BalanceSheetView({ periods }: { periods: FiscalPeriod[] }) {
  ...
  if (loading) return <TableSkeleton rows={6} />;
  if (error) return <FinanceErrorState message={error} onRetry={() => void run(asOf)} />;
  if (!data) return null;
  ...
  return (
    <div className="space-y-5">
      ...
      />
      <ReportToolbar
        fields={[{ id: "bs-asof", label: "As of", value: asOf, onChange: setAsOf }]}
        onRun={() => void run(asOf)}
        running={loading}
      />
      <StatementTable
        rows={[
          ...
          { kind: "total", label: "Total assets", amount: data.total_assets },
          ...
          { kind: "total", label: "Total liabilities", amount: data.total_liabilities },
          ...
          { kind: "total", label: "Total equity", amount: data.total_equity },
          {
            kind: "net",
            label: "Balance check (assets − liabilities − equity)",
            amount:
              toMoney(data.total_assets) - toMoney(data.total_liabilities) - toMoney(data.total_equity),
          },
        ]}
...
export function FinanceReports() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div role="tablist" aria-label="Financial reports" className="inline-flex rounded-lg border border-border bg-card p-0.5">
        {REPORT_TABS.map((tab) => (
          <button
            key={tab.key}
            type="button"
      ...
      {report === "trial-balance" ? <TrialBalanceView periods={periods} /> : null}
      {report === "profit-and-loss" ? <ProfitAndLossView periods={periods} /> : null}
      {report === "balance-sheet" ? <BalanceSheetView periods={periods} /> : null}
    </div>
  );
}
