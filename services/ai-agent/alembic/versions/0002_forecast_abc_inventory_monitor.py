...
def upgrade() -> None:
    ...
    op.create_table(
        "ai_forecasts",
        sa.Column("tenant_id", UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="CASCADE"), primary_key=True, nullable=False),
        ...
        sa.Column("avg_daily_demand", sa.Numeric(18, 4), nullable=False, server_default=sa.text("0")),
        ...
        sa.UniqueConstraint("tenant_id", "product_id", "horizon_weeks", name="uq_ai_forecast_product_horizon"),
    )
    ...
    op.execute(
        "ALTER TABLE ai_abc_classifications ENABLE ROW LEVEL SECURITY"
    )
    op.execute(
