...
it derives the tenant slug via the centralized ``TenantResolver``, verifies
the tenant in the shared database, cross-checks the verified JWT against it,
and populates TenantContext. Downstream code consumes TenantContext instead
of re-reading headers or parsing the Host again.
...
from ai_agent.core.constants import SKIP_AUTH_PATHS
...
from ai_agent.core.tenant_resolver import derive_tenant_slug
from skyrict_common.exceptions import TokenExpiredError, TokenInvalidError

logger = structlog.get_logger("ai_agent.middleware")
...
class TenantContextMiddleware(BaseHTTPMiddleware):
    ...
    Flow (single source of truth — no other layer re-resolves the tenant):
      ...
      2. Derive the tenant slug from the routing layer (Host / X-Tenant-Slug).
         The core proxy forwards the original headers, so resolution behaves
         identically whether called directly or through the monolith.
