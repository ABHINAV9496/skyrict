"""Unit tests for the per-tenant suggestion-expiry job.

Exercises ``expire_all_tenants`` orchestration with fakes for the session
factory and a stub repository that records the pinned ``TenantContext`` at
call time; the repository UPDATE path is covered by the suggestion repository
tests.
"""
...
from ai_agent.core.jobs.suggestion_expiry import expire_all_tenants
    ...
    def factory(session: object) -> object:
        class _Repo:
            async def expire_stale(self, **kwargs: object) -> int:
                ...
                record["expiry_days"].append(kwargs["expiry_days"])
...
class TestExpireAllTenants:
    async def test_skips_when_no_active_tenants(self) -> None:
        record: dict = {"ctx": [], "slug": [], "expiry_days": [], "commits": []}
        total = await expire_all_tenants(
    ...
    async def test_expires_each_tenant_with_pinned_context(self) -> None:
        record: dict = {"ctx": [], "slug": [], "expiry_days": [], "commits": []}
