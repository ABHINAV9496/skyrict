...
async def _accrue(
    service: LeaveService, repo: FakeHrRepository, *, year: int = 2024, qty: int = 10
) -> None:
...
class TestRule1ApproveWritesMovement:
    async def test_approve_writes_minus_days_and_seeds_balance(self) -> None:
        ...
        request, balance = await service.approve(
...
class TestRule5Cancel:
    async def test_cancel_only_from_approved_restores_days(self) -> None:
        ...
        cancellation = [
            m for m in repo.movements if m.ref_type == "cancellation" and m.ref_id == str(req.id)
        ]
