...
reserved/platform slugs) lives in one testable module. Downstream services and
repositories never re-read headers or re-parse the Host.

Two routing contracts:
  ...
  - dev/test: the slug comes from the X-Tenant-Slug header injected by the
    local nginx (infra/nginx/dev.conf), which always overwrites client input.

Reserved/platform slugs (RESERVED_SLUGS) never resolve to a tenant: a host
like web.skyrict.com is a platform surface (marketing, signin, docs, ...),
not a tenant subdomain, so those hosts return None and are never looked up
as tenants.
"""
...
class TenantResolver:
    ...
    def resolve_from_header(self, slug: str | None) -> str | None:
        ...
        The header is injected by the local nginx (infra/nginx/dev.conf),
        which always overwrites client input, so it is trusted in dev/test
        only. Reserved slugs never resolve to a tenant.
        """
...
def resolve_tenant_slug_from_host(host: str, *, base_domain: str) -> str | None:
    """Backwards-compatible module function wrapping ``TenantResolver``.

    Kept so existing callers/tests can resolve without an instance.
    """
    return TenantResolver(base_domain=base_domain).resolve_from_host(host)


def derive_tenant_slug(request: Request) -> str | None:
    """Backwards-compatible module function using the live ``settings``.
